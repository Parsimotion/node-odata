...
var icontains = function icontains(query, fnKey, odataOperator, value) {
  var _fnKey$substring$spli5 = fnKey.substring(fnKey.indexOf('(') + 1, fnKey.indexOf(')')).split(',');

  var _fnKey$substring$spli6 = _slicedToArray(_fnKey$substring$spli5, 2);

  var key = _fnKey$substring$spli6[0];
  var target = _fnKey$substring$spli6[1];
  var _ref3 = [key.trim(), target.trim()];
  key = _ref3[0];
  target = _ref3[1];
...
exports.default = { indexof: indexof, year: year, contains: contains, icontains: icontains };